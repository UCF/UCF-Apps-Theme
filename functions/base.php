...
function bootstrap_menus() {
	class Bootstrap_Walker_Nav_Menu extends Walker_Nav_Menu {
			...
			function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
				...
				$item_output .= ($args->has_children) ? ' <i class="icon-chevron-down icon-white"></i></a>' : '</a>';
...
function get_featured_image_url($post) {
	if(has_post_thumbnail($post) && ($thumbnail_id = get_post_thumbnail_id($post)) && ($image = wp_get_attachment_image_src($thumbnail_id, 'full'))) {
		return $image[0];
	}
	return False;
}
...
function _save_meta_data($post_id, $meta_box){
  ...
  /**
   * Special saving method for Sliders:
   *
   **/
	if (post_type_exists('slider') && post_type($post_id) == 'slider') {
